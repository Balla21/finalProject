...
import * as ProductPage from "../viewpage/admin_product_page.js";

let imageFile2Upload;

export function resetImageSelection(){
    imageFile2Upload = null;
    Element.imageTagProduct.src = "";
}
...
export function addEventListeners(){
    ...
    Element.formImageAddButton.addEventListener("change", e => {
        ...
        reader.onload = () => Element.imageTagProduct.src = reader.result;
...
async function addNewProduct(e){
    ...
    //Check image File to upload
    const errors = product.validate(imageFile2Upload);

    if(errors){
        if(errors.name) Element.formAddProductError.name.innerHTML = errors.name;
        if(errors.price) Element.formAddProductError.price.innerHTML = errors.price;
        if(errors.summary) Element.formAddProductError.summary.innerHTML = errors.summary;
        if(errors.image) Element.formAddProductError.image.innerHTML = errors.image;
        return;
    }
